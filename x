...
local function UpdateFOV()
    ...
    -- Accessing the FOV settings from the new CamLock table
    Circle.Visible = getgenv().CamLock.Normal.Enabled -- Assuming you want the circle to be visible only when CamLock is enabled
    Circle.Radius = getgenv().CamLock.Normal.Radius -- Using the radius from the new table
...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == getgenv().Target.Keybind:lower() then  -- Start targeting
        if getgenv().CamLock.Normal.Enabled then
            if getgenv().CamLock.Normal.mode == "toggle" then
            ...
            elseif getgenv().CamLock.Normal.mode == "hold" then
                IsTargeting = true
                TargetPlayer = ClosestPlrFromMouse()
            end
        end
    elseif Key:lower() == getgenv().Target.UntargetKeybind:lower() then  -- Untarget
        IsTargeting = false
        TargetPlayer = nil
    end
end)
...
local function updateFov()
    local settings = getgenv().Silent.AimSettings.FovSettings  -- Updated to use the new structure
    clearFovParts()
...
local function isPartInFovAndVisible(part)
    ...
    return onScreen and distance <= getgenv().Silent.AimSettings.FovSettings.FovRadius -- Updated to reference the correct FOV settings
end
...
local function isPartVisible(part)
    if not getgenv().Silent.Normal.WallCheck then  -- Updated to use the new structure
        return true
    end
...
local OldPredictionY = getgenv().Silent.Normal.Prediction  -- Updated reference
local function GetVelocity(player, part)
    if player and player.Character then
        local velocity = player.Character[part].Velocity
        if velocity.Y < -30 and getgenv().Silent.Normal.Resolver then  -- Updated reference
            getgenv().Silent.Normal.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and getgenv().Silent.Normal.Resolver then  -- Updated reference
...
local function toggleFeature()
    getgenv().Silent.Normal.Enabled = not getgenv().Silent.Normal.Enabled  -- Updated reference
    local status = getgenv().Silent.Normal.Enabled and "Forbidden Enabled" or "Forbidden Disabled"  -- Updated reference
    sendNotification("Forbidden Notifications", status, "rbxassetid://17561420493")
    
    if not getgenv().Silent.Normal.Enabled then  -- Updated reference
        getgenv().Silent.AimSettings.FovSettings.FovVisible = false  -- Ensure FOV is hidden when disabled
        clearFovParts()
    end
end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    -- Check if the Target keybind was released
    if input.KeyCode == Enum.KeyCode[getgenv().Target.Keybind:upper()] then
        if getgenv().CamLock.Normal.mode == "hold" then
            holdingMouseButton = true
            IsTargeting = true  -- Start targeting when the target key is released
            TargetPlayer = ClosestPlrFromMouse()  -- Set the target player
        end
    end

    -- Check if the Untarget keybind was released
    if input.KeyCode == Enum.KeyCode[getgenv().Target.UntargetKeybind:upper()] then
        IsTargeting = false  -- Stop targeting when the untarget key is released
        TargetPlayer = nil  -- Clear the target
    end

    -- Check if the CamLock hold keybind was released
    if input.KeyCode == Enum.KeyCode[getgenv().CamLock.Normal.Keybind:upper()] and getgenv().CamLock.Normal.mode == "hold" then
...
Run_Service.RenderStepped:Connect(function()
    if getgenv().Silent.Active and IsTargeting then  -- Only work when targeting is engaged
        updateFov()  -- Call updateFov to refresh visibility
        TargetPlayer = GetClosestPlr()  -- Get the closest player instead of using a static Target variable
        ...
        if TargetPlayer and TargetPlayer.Character then
            ...
            if closestPart and closestPoint then
                ...
                Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().Silent.WeaponStats[closestPart.Name] * getgenv().Silent.Normal.Prediction)  -- Ensure using correct prediction
            end
        end
    else
        getgenv().Silent.AimSettings.FovSettings.FovVisible = false  -- Ensure FOV is hidden if not targeting
    end
end)
...
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().Silent.Normal.Enabled then  -- Updated reference
        ...
        tool.Activated:Connect(function()
            if Target and Target.Character and tick() - lastToolUse > 0.1 then  -- Debounce for 0.1 seconds
                ...
                if closestPart and closestPoint then
                    ...
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().Silent.Normal.Prediction)  -- Updated reference
                end
            end
        end)
    end
...
if getgenv().Silent.Adjustment.Checks.NoGroundShots == true then
    local function CheckNoGroundShots(Plr)
        if getgenv().Silent.Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(getgenv().Silent.Normal.HitPart)  -- Ensure this references the correct part based on your setup
                if TargetVelv5 then
